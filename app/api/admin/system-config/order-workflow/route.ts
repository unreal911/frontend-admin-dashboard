...
export async function GET() {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/system-config/order-workflow`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo cargar la configuracion.' }, { status: 502 });
  }
...
export async function PATCH(request: Request) {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo actualizar la configuracion.' }, { status: 502 });
  }
