...
async function proxySunatRoute(
  request: Request,
  ...
  const upstreamUrl = query
    ? `${getAdminApiUrl()}/sunat/${routePath}?${query}`
    : `${getAdminApiUrl()}/sunat/${routePath}`;
  ...
  if (!upstream) {
    return NextResponse.json(
      { success: false, message: 'No se pudo completar la solicitud a SUNAT.' },
      { status: 502 },
    );
  }
