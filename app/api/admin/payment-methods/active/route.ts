...
export async function GET() {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/payment-methods/active`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudieron consultar metodos de pago activos.' }, { status: 502 });
  }
