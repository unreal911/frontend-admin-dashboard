...
export async function PUT(request: Request, context: RouteContext) {
  ...
  if (!Number.isInteger(paymentMethodId) || paymentMethodId < 1) {
    return NextResponse.json({ success: false, message: 'Id de metodo de pago invalido.' }, { status: 400 });
  }
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/payment-methods/${paymentMethodId}`, {
    method: 'PUT',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo actualizar el metodo de pago.' }, { status: 502 });
  }
