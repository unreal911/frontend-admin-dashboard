...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/user-activities?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo cargar la auditoria de usuarios.' }, { status: 502 });
  }
