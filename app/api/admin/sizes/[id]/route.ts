...
export async function PUT(request: Request, context: RouteContext) {
  ...
  if (!Number.isInteger(sizeId) || sizeId < 1) {
    return NextResponse.json({ success: false, message: 'Id de talla invalido.' }, { status: 400 });
  }
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/size/${sizeId}`, {
    method: 'PUT',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo actualizar la talla.' }, { status: 502 });
  }
