...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/size?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudieron consultar tallas.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/size`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo crear la talla.' }, { status: 502 });
  }
