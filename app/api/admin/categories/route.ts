...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/categorie?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo consultar categorias.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/categorie`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo crear la categoria.' }, { status: 502 });
  }
