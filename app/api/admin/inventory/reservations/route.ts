...
export async function GET(request: Request) {
  ...
  const upstreamUrl = query
    ? `${getAdminApiUrl()}/inventory/reservations?${query}`
    : `${getAdminApiUrl()}/inventory/reservations`;
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo cargar trazabilidad.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/inventory/reservations`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${auth.token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo crear reserva.' }, { status: 502 });
  }
