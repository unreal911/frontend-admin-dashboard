...
export async function GET(request: Request) {
  ...
  const upstreamUrl = query
    ? `${getAdminApiUrl()}/inventory/movements?${query}`
    : `${getAdminApiUrl()}/inventory/movements`;
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudieron cargar movimientos.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/inventory/movements`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${auth.token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo registrar movimiento.' }, { status: 502 });
  }
