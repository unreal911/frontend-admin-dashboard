...
export async function GET(request: Request) {
  ...
  const upstreamUrl = query
    ? `${getAdminApiUrl()}/orders?${query}`
    : `${getAdminApiUrl()}/orders`;
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudieron cargar los pedidos.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/orders`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${auth.token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo crear el pedido.' }, { status: 502 });
  }
