...
export async function GET() {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/permissions`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo cargar el catalogo de permisos.' }, { status: 502 });
  }
