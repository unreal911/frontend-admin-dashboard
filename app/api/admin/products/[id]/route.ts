...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!Number.isInteger(productId) || productId < 1) {
    return NextResponse.json({ success: false, message: 'Id de producto invalido.' }, { status: 400 });
  }
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/products/${productId}`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo consultar el producto.' }, { status: 502 });
  }
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo actualizar el producto.' }, { status: 502 });
  }
...
// Compatibilidad con cliente que use PUT.
