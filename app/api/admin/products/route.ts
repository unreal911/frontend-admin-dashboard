...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/products?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudieron consultar productos.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${getAdminApiUrl()}/products`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  if (!upstream) {
    return NextResponse.json({ success: false, message: 'No se pudo crear el producto.' }, { status: 502 });
  }
